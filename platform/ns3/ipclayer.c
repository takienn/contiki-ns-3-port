...
//Semaphore objects
sem_t *sem_in, *sem_time, *sem_timer; // *sem_out
//sem_t *sem_timer_go,*sem_timer_done ;
sem_t *sem_traffic_go, *sem_traffic_done;
...
void ipc_init(char *id) {
	// The size of the message, its size, and 1 more for the type (data or timer)
	int newSize = sizeof_size_t + traffic_size +1;
...
//		perror("contiki shm_open(shm_timer)");
	...
	// just to be sure, if we have REALLY small packets
	if(newSize < (size_time + 2))
		newSize = size_time + 2;

	if (ftruncate(shm_out, 1310) == -1)
...
//		perror("contiki ftruncate(shm_timer) error");
	 ...
	 */
	addr_in = mmap(NULL, traffic_size + sizeof_size_t, PROT_READ | PROT_WRITE,
			MAP_SHARED, shm_in, 0);
	addr_time = mmap(NULL, size_time, PROT_READ | PROT_WRITE, MAP_SHARED,
			shm_time, 0);

	addr_out = mmap(NULL, newSize, PROT_READ | PROT_WRITE,
			MAP_SHARED, shm_out, 0);

//	addr_out = mmap(NULL, traffic_size + sizeof_size_t, PROT_READ | PROT_WRITE,
//			MAP_SHARED, shm_out, 0);
...
/////// Semaphores to make sure traffic is handled by ns3 ////////////
	...
	if ((sem_traffic_go = sem_open(sem_traffic_go_name, 0)) == SEM_FAILED)
		perror("contiki sem_open(sem_traffic_go) failed");
	if ((sem_traffic_done = sem_open(sem_traffic_done_name, 0)) == SEM_FAILED)
		perror("contiki sem_open(sem_traffic_done) failed");
...
void ipc_write(uint8_t *buf, size_t len) {
	uint8_t typeBuffer = 1; // type of the message on the buffer, packet
...
//		perror("contiki sem_wait() error");
	...
	// wait for ns-3 to be ready to accept a new packet
	sem_wait(sem_traffic_go);

	// writing data type
	memcpy(addr_out, &typeBuffer, 1);

	// writing data size
	memcpy(addr_out + 1, &len, sizeof_size_t);

	// now writing data of that size
	memcpy(addr_out + sizeof_size_t + 1, buf, len);

	//PRINTF("Contiki wrote packet of size %d\n", len);
#if DEBUG
	puts("ipc_write\n");
	fwrite(addr_out + sizeof_size_t + 1, 1, len, stdout);
	puts("\n");
	fflush(stdout);
#endif
...
void ipc_settimer(uint64_t interval, uint8_t type) {
	uint8_t typeBuffer = 0; // type of the message on the buffer, packet
...
//		perror("contiki sem_wait() error");
	...
	// wait for ns-3 to be ready to accept a new timer
	sem_wait(sem_traffic_go);

	// writing data type
	memcpy(addr_out, &typeBuffer, 1);

	// writing the type of the timer
	memcpy(addr_out + 1, &type, 1);

	// writes the timer itself
	memcpy(addr_out + 2, &interval, 8);
...
//		perror("contiki sem_wait() error");
	...
	// warns that the traffic is done
	sem_post(sem_traffic_done);
	//PRINTF("Contiki waiting to proceed after writing a packet\n");

	// waits for ns-3 to grab the timer from the memory
	sem_wait(sem_traffic_go);
